package edu.wm.cs.cs301.brukeamare.extra;

import static edu.wm.cs.cs301.brukeamare.gui.Apptesting.tag;

import android.util.Log;

import edu.wm.cs.cs301.brukeamare.extra.Constants.UserInput;
import edu.wm.cs.cs301.brukeamare.extra.Robot.Direction;

import java.io.File;
import java.util.logging.Logger;

import edu.wm.cs.cs301.brukeamare.generation.CardinalDirection;
import edu.wm.cs.cs301.brukeamare.generation.Floorplan;
import edu.wm.cs.cs301.brukeamare.generation.Maze;
import edu.wm.cs.cs301.brukeamare.generation.ReliableRobot;
import edu.wm.cs.cs301.brukeamare.generation.ReliableSensor;
import edu.wm.cs.cs301.brukeamare.generation.SmartWallFollower;
import edu.wm.cs.cs301.brukeamare.generation.SmartWizard;
import edu.wm.cs.cs301.brukeamare.generation.UnreliableRobot;
import edu.wm.cs.cs301.brukeamare.generation.UnreliableSensor;
import edu.wm.cs.cs301.brukeamare.generation.WallFollower;
import edu.wm.cs.cs301.brukeamare.generation.Wizard;
import edu.wm.cs.cs301.brukeamare.gui.MazePanel1;
import edu.wm.cs.cs301.brukeamare.gui.PlayAnimationActivity;
import edu.wm.cs.cs301.brukeamare.gui.PlayManuallyActivity;
...
//    private Control control;
	...
	public float batterlevelnow=3500;
	public Robot robos;
	...
	public PlayAnimationActivity playAnimationActivity= null;
	public PlayManuallyActivity playManuallyActivity= null;
	...
	public boolean SmartWizardmode=false;
     ...
     */
    public void start( MazePanel1 panel) {
    ...
    protected void runWiz(){
    		...
    		try{
				playAnimationActivity.startsensor();
				playAnimationActivity.updatebatt(robo);
//    			wizardry=playAnimationActivity.drive2exit(robodriver, robo);
				wizardry=robodriver.drive2Exit();
    			pathlengthy=robodriver.getPathLength();
				batterylevelset=robo.getBatteryLevel();
				Log.v(tag,"animation"+batterylevelset);


			}catch(Exception e) {
    			pathlengthy=robodriver.getPathLength();
    			wizardCrashedorLost= true;
				batterylevelset=robo.getBatteryLevel();
				switchFromPlayingToWinning(pathlengthy, batterylevelset);
    			return;
    		}
			if (!wizardry){
				pathlengthy=robodriver.getPathLength();
    ...
    protected void runSmartWiz(){
		...
		((SmartWizard) robodriver).currentState=this;
		robodriver.setRobot(robo);
		robodriver.setMaze(maze);
		//control.setRobotAndDriver(robo, robodriver);
		for(Direction i: Direction.values()) {
...
//			wizardry=playAnimationActivity.drive2exit(robodriver, robo);
    ...
    protected void runWallfollower(){
        ...
        wallfollowerCrashedorLost= false;
		...
		((WallFollower) robodriver).currentState=this;
		robodriver.setRobot(robo);
		robodriver.setMaze(maze);
		//control.setRobotAndDriver(robo, robodriver);
...
//			wizardry=playAnimationActivity.drive2exitunreliable(robodriver, robo);
			playAnimationActivity.updatebatt(robo);
			wizardry=robodriver.drive2Exit();
			pathlengthy=robodriver.getPathLength();
			batterylevelset=robo.getBatteryLevel();
			...
			pathlengthy=robodriver.getPathLength();
			wallfollowerCrashedorLost= true;
			batterylevelset=robo.getBatteryLevel();
			switchFromPlayingToWinning(pathlengthy, batterylevelset);
			return;
    ...
    protected void runSmartWallfollower(){
		...
		((SmartWallFollower) robodriver).currentState=this;
		robodriver.setRobot(robo);
		robodriver.setMaze(maze);
		//control.setRobotAndDriver(robo, robodriver);
...
//			wizardry=playAnimationActivity.drive2exitunreliable(robodriver, robo);
     ...
     */
    public void switchFromPlayingToWinning(int pathLength, float battery) {
		Log.v(tag,"switching "+battery);
		if(wizardCrashedorLost || wallfollowerCrashedorLost){
			playAnimationActivity.crashed(pathLength, battery);
		}
		else{
			if(playManuallyActivity==null){
				playAnimationActivity.reachedexit(pathLength, battery);
			}
			else{
				playManuallyActivity.reachedexit(pathLength);
			}
		}
...
//        currentState.start(control, panel);
     ...
     * Method requires {@link #start( MazePanel1) start} to be
     * called before.
    ...
    public boolean handleUserInput(UserInput userInput, int value) {
        ...
        case UP: // move forward
            ...
            walk(1);
			pathlengthy=pathlengthy+1;
            // check termination, did we leave the maze?
			if(playAnimationActivity!=null) {
				if (isOutside(px , py)) {
					// TODO: provide actual path length
					switchFromPlayingToWinning(pathlengthy, robos.getBatteryLevel());
				}
			}
			else if(px==maze.getExitPosition()[0]&& py==maze.getExitPosition()[1]) {
				CardinalDirection card = getCurrentDirection();
				int[] arr = card.getDxDyDirection();
				if (isOutside(px + arr[0], py + arr[1]) && !maze.hasWall(px, py, card)) {

					// TODO: provide actual path length
					switchFromPlayingToWinning(pathlengthy, batterlevelnow);
				}
			}
