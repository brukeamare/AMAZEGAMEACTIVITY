...
import edu.wm.cs.cs301.brukeamare.generation.CardinalDirection;
import edu.wm.cs.cs301.brukeamare.extra.ColorTheme.MazeColors;
...
//     * @param markerFont    The font used for the markers.
     ...
     */
    public void paintComponent(MazePanel1 g) {
...
//        g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
     ...
     */
	private void drawArms(final MazePanel1 g2, final int length, final int width) {
     ...
     * @param g2 The graphics object to draw on
     */
	private void drawBackground(final MazePanel1 g2) {
	 ...
	 */
	private void drawArmWest(MazePanel1 g2, int length, int width, int[] x, int[] y) {
	 ...
	 * @param y For the y coordinates of the triangle points, first entry given and fixed
	 */
	private void drawArmEast(MazePanel1 g2, int length, int width, int[] x, int[] y) {
	 ...
	 * @param y For the y coordinates of the triangle points, first entry given and fixed
	 */
	private void drawArmSouth(MazePanel1 g2, int length, int width, int[] x, int[] y) {
		// x[0] and y[0] are already set to the coordinates of the center point
		// set coordinates for 2nd and 3rd point and draw filled triangle
		x[1] = centerX;
        y[1] = centerY + length;
	 ...
	 * @param y For the y coordinates of the triangle points, first entry given and fixed
	 */
	private void drawArmNorth(MazePanel1 g2, int length, int width, int[] x, int[] y) {
	 ...
	 * @param width
	 */
	private void drawBorderCircle(MazePanel1 g2, int width) {
	 ...
	 */
	private void drawDirectionMarker(MazePanel1 g2, int width) {
		...
		drawMarker(g2, centerX-8, centerY - offset+8, "N");

		g2.setColor((CardinalDirection.East == currentDir) ?
				(ColorTheme.getColor(MazeColors.COMPASSROSE_MARKER_COLOR_CURRENTDIRECTION)).toArgb() :
				(ColorTheme.getColor(MazeColors.COMPASSROSE_MARKER_COLOR_DEFAULT)).toArgb());
		drawMarker(g2, centerX + offset-8, centerY+8, "E");
		...
		g2.setColor((CardinalDirection.North == currentDir) ?
				(ColorTheme.getColor(MazeColors.COMPASSROSE_MARKER_COLOR_CURRENTDIRECTION)).toArgb() :
				(ColorTheme.getColor(MazeColors.COMPASSROSE_MARKER_COLOR_DEFAULT)).toArgb());
		drawMarker(g2, centerX-8, centerY + offset+8, "S");

		g2.setColor((CardinalDirection.West == currentDir) ?
				(ColorTheme.getColor(MazeColors.COMPASSROSE_MARKER_COLOR_CURRENTDIRECTION)).toArgb() :
				(ColorTheme.getColor(MazeColors.COMPASSROSE_MARKER_COLOR_DEFAULT)).toArgb());
		drawMarker(g2, centerX - offset-8, centerY+8, "W");
	 ...
	 */
    private void drawMarker(MazePanel1 g2, float x, float y, String str) {
